...
```
> Дело в том, что len измеряет строку по байтам, а не по символам. Так же важно аккуратнее работать с индексированием строк (вызвав `string[5]` можно получить не то что ожидалось). Лучше использовать `for` или `split`.
